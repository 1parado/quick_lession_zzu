...
1. 选课系统的短信配置页面
   ...
   2. 智能定时发送：对于教师，在到达发布时间的前Xmin进行发送短信（默认10min）		1
   ...
   3. 可以选择不发送（关闭发送短信的功能）(Redis,默认是开启)		1

​		在系统启动初始化时，在Redis中设置一个开启短信功能的参数，在执行定时任务时，检查该参数

​	

BUG：数据库中的时间与页面设置的不一致
...
BUG：修改完毕后直接发布？（管理员端/教师端）



2. 定时通知（公告到达发布时间时通知教师（必须有手机号），发布公告后通知学生）

   ​	利用SpringBoot的@Schedule

   

   智能防骚扰，在2小时内不会重复的给同一个人发布消息			1

   

   发布时通知学生（AOP）（后期可以利用AOP进行日志控制）
   ...
   可以在选中模板页面添加一个示例，就是说管理员编辑了一个模板以后，会举一个例子（张三...）将短信内容显示在页面上，已验证模板是否正确

   

   notify_time表 不要重复新建同样的draft_id
   ...
   2. 该接口负责将短信发送给用户
      ...
      注意：在定时中顺便检查notify_time表删除已经无用的数据（时间在此时之前的）
