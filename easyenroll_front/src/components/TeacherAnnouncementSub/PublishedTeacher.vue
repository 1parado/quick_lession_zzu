...
<template>
  <el-table :data="announcementTableData" stripe style="width: 100%"  v-if="announcementTableData.length > 0" >
    ...
    <el-table-column label="发布时间" wixdth="180" >
  ...
  />
  <el-empty :image-size="200" v-else style="margin-top: 175px" description='暂无已发布公告'/>
  ...
  <el-dialog v-model="AnnouncementDetailUpdateVisible" title="修改公告信息" width="1000">
    <div>
      <el-form :model="announcementData" :rules="rules" ref="updateFormRef" label-position="left" label-width="80px" >
        <el-form-item label="发布人" prop="publicName" >
          <el-input v-model="announcementData.publicName" disabled />
        </el-form-item>
        <el-form-item label="发布时间" >
          <el-input :value="formatTime(announcementData.publishTime)" disabled />
        </el-form-item>
...
const fetchData = async () => {
  try {
    const response = await axios.get('/ann/announcement/teacher/' + 2 + '/' + sessionStorage.getItem('account'), {
      params: {
