<template>
  ...
  <el-button type="primary" plain @click="openInsertDialog()" style="margin-bottom: 30px">新增公告</el-button>

  <el-button type="primary" plain @click="router.push('/smsConfig')" style="margin-left: 1480px;margin-bottom: 30px">短信配置</el-button>
...
const handleClick = (tab: TabsPaneContext, event: Event) => {
  ...
  switch(tab.paneName) {
    case 'ALL':
      router.push('/announcementManage')  // 跳转到已发布页面路由
      break
    case 'PUBLISHED':
      router.push('published')  // 跳转到已发布页面路由
      break
    case 'DRAFT':
      router.push('draft')      // 跳转到草稿箱页面路由
      break
    case 'DELETED':
      router.push('deleted')    // 跳转到已删除页面路由
      break
  }
}
...
const insertAnnouncement = async () => {
  ...
  try {
    ...
    if (response.data.code === 200){
      ...
      //router.push('draft');
      router.push({ path: 'draft', query: { t: Date.now() } });
    } else {
...
const sendAnnouncement = async () => {
  ...
  try {
    ...
    console.log(response);
    if (response.data.code === 200){
      ElMessage.success("保存成功发布成功")
      AnnouncementInsertDialogVisible.value = false;
      //router.push('published');
      router.push({ path: 'published', query: { t: Date.now() } });
    } else {
